<template>
  ...
  >
    ...
    <label v-if="$slots.default" class="lbz-checkbox__label" :for="`${id}`"
      ><slot
    /></label>
  </div>
</template>
...
<script lang="ts">
  ...
  export default class LbzCheckbox extends Vue {
    ...
    @Prop({ type: String, default: `lbz-checkbox-${lbzfRandomId()}` })
    private id!: string;
    ...
    @Prop({ type: [Boolean, Number, String], default: undefined })
