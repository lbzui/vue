<template>
  ...
  >
    ...
    <label v-if="$slots.default" class="lbz-radio__label" :for="`${id}`"
      ><slot
    /></label>
  </div>
</template>
...
<script lang="ts">
  ...
  export default class LbzRadio extends Vue {
    ...
    @Prop({ type: String, default: `lbz-radio-${lbzfRandomId()}` })
    private id!: string;
    // [required]value: true, false, x, 'x'
    @Prop({ type: [Boolean, Number, String], required: true }) private value!:
      | boolean
      | number
      | string;
