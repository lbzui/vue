<template>
  ...
  >
    ...
    <label v-if="$slots.default" class="lbz-switch__label" :for="`${id}`"
      ><slot
    /></label>
  </div>
</template>
...
<script lang="ts">
  ...
  export default class LbzSwitch extends Vue {
    ...
    @Model('change', { type: [Boolean, Number, String], required: true })
    private mchecked!: boolean | number | string;

    // id: 'lbz-switch-x' (default), 'x'
    @Prop({ type: String, default: `lbz-switch-${lbzfRandomId()}` })
    private id!: string;
    // true-value: true (default), false, x, 'x'
    @Prop({ type: [Boolean, Number, String], default: true })
    private trueValue!: boolean | number | string;
    // false-value: true, false (default), x, 'x'
    @Prop({ type: [Boolean, Number, String], default: false })
    private falseValue!: boolean | number | string;
    ...
    get cgetStateAttrs(): LbzStateAttributes {
      return {
        ...
        type:
          ['primary', 'secondary', 'error'].includes(this.onBackground) ||
          this.cisChecked
            ? 'primary'
            : '',
        background: this.cisChecked
          ? this.color || 'secondary'
          : this.onBackground
          ? `on-${this.onBackground}`
          : '',
    ...
    private fclick(e: MouseEvent) {
      return this.cisChecked ? this.falseValue : this.trueValue;
    }
  }
</script>
